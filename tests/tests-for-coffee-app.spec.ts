...
test('AS-0003 Order with 3 products and promotion product',
   {
    tag: ["@Regression"],
    annotation: {
      type: "description",
...
test('AS-0004 Check that SKIP button not add new promo product to the Cart',

    {
    tag: ["@Smoke"],
    annotation: {
      type: "description",
      description: "Check that SKIP button not add new promo product to the Cart"
    },
  },
  ...
  await expect(page.locator('#app')).toContainText('Cappuccino x 1+-Espresso x 1+-Espresso Macchiato x 1+-');
});
...
test('AS-0005 Each subsequent discounted product (after first one) is added after adding 2 more products to the cart.',
   {
    tag: ["@Regression"],
    annotation: {
      type: "description",
...
test('AS-0006 User can delete all products from the Cart and Cart is displayed as empty', 
    {
    tag: ["@Smoke"],
    annotation: {
      type: "description",
  ...
  await page.getByRole('link', { name: 'Cart page' }).click();
  await expect(page.locator('div').filter({ hasText: 'Americano x 1+-Flat White x 1' }).nth(1)).toBeVisible();
  await page.getByRole('button', { name: 'Remove all Americano' }).click();
  await expect(page.locator('div').filter({ hasText: 'Flat White x 1+-Total: $18.' }).nth(1)).toBeVisible();
...
test('AS-0007 User can increase and reduce quantity of products in the Cart',
    {
    tag: ["@Smoke"],
    annotation: {
      type: "description",
  ...
  await page.locator('[data-test="Flat_White"]').click();
  await page.getByRole('link', { name: 'Cart page' }).click();
  await expect(page.locator('div').filter({ hasText: 'Espresso Macchiato x 1+-Flat' }).nth(1)).toBeVisible();
